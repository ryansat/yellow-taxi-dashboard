...
function Charts({ fuelData }) {
  ...
  const fuelStats = useMemo(() => {
    ...
    const fromDateParts = fuelData.FromDate.split(" ")[0].split("-");
    const toDateParts = fuelData.ToDate.split(" ")[0].split("-");
    ...
    return {
      ...
      totalRefuelAmount: refuels.reduce((sum, refuel) => sum + refuel.Amount, 0),
      averageRefuelAmount: refuels.length > 0
          ? refuels.reduce((sum, refuel) => sum + refuel.Amount, 0) / refuels.length
          : 0,
      averageRefuelDuration: refuels.length > 0
          ? refuels.reduce((sum, refuel) => sum + refuel.Duration, 0) / refuels.length
          : 0,
    };
  }, [fuelData, refuels]);
      ...
      <div className="fuel-charts-container">
        ...
        {fuelStats && (
            <div className="fuel-stats">
              ...
              <p>Average Refuel Amount: {fuelStats.averageRefuelAmount.toFixed(3)} L</p>
              <p>Average Refuel Duration: {(fuelStats.averageRefuelDuration * 60).toFixed(2)} minutes</p>
            </div>
        )}
